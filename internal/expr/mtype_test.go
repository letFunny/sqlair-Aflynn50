...
type wrongMap map[int]any
type rightMap map[string]any
...
func (s *ExprSuite) TestValidPrepareWithMtype(c *C) {
	testList := []struct {
		sql      string
		args     []any
		expected string
...
func (s *ExprSuite) TestValidPrepareWithRightMap(c *C) {
	testList := []struct {
		sql      string
		args     []any
		expected string
	}{{
		sql:      "SELECT street FROM t WHERE x = $rightMap.street",
		args:     []any{rightMap{}},
		expected: "SELECT street FROM t WHERE x = @sqlair_0",
	}, {
		sql:      "SELECT street FROM t WHERE x, y = ($rightMap.street, $Person.id)",
		args:     []any{Person{}, rightMap{}},
		expected: `SELECT street FROM t WHERE x, y = (@sqlair_0, @sqlair_1)`,
	}, {
		sql:      "SELECT p FROM t WHERE x = $rightMap.id",
		args:     []any{rightMap{}},
		expected: "SELECT p FROM t WHERE x = @sqlair_0",
	}, {
		sql:      "INSERT INTO person (id, postalcode) VALUES ($rightMap.id, $Address.id)",
		args:     []any{Address{}, rightMap{}},
		expected: `INSERT INTO person (id, postalcode) VALUES (@sqlair_0, @sqlair_1)`,
	}, {
		sql:      "SELECT * AS (&rightMap.manager_name, &Person.*, &Address.id) FROM t",
		args:     []any{Address{}, Person{}, rightMap{}},
		expected: "SELECT manager_name AS _sqlair_0, address_id AS _sqlair_1, id AS _sqlair_2, name AS _sqlair_3, id AS _sqlair_4 FROM t",
	}}
...
func (s *ExprSuite) TestValidCompleteWithMtype(c *C) {
	testList := []struct {
		input          string
		prepareArgs    []any
		completeArgs   []any
...
func (s *ExprSuite) TestValidCompleteWithMap(c *C) {
	testList := []struct {
		...
		"SELECT * AS &Address.* FROM t WHERE x = $rightMap.fullname",
		[]any{Address{}, rightMap{}},
		[]any{rightMap{"fullname": "Jimany Johnson"}},
		[]any{sql.Named("sqlair_0", "Jimany Johnson")},
	}, {
		"SELECT foo FROM t WHERE x = $rightMap.street, y = $Person.id",
		[]any{Person{}, rightMap{}},
		[]any{Person{ID: 666}, rightMap{"street": "Highway to Hell"}},
		[]any{sql.Named("sqlair_0", "Highway to Hell"), sql.Named("sqlair_1", 666)},
	},
	}
	for _, test := range testList {
...
func (s *ExprSuite) TestCompleteWithWrongMap(c *C) {
	testList := []struct {
		...
		"SELECT foo FROM t WHERE x = $M.street, y = $Person.id",
		[]any{Person{}, sqlair.M{}},
		[]any{Person{ID: 666}, wrongMap{5: "Highway to Hell"}},
		[]any{sql.Named("sqlair_0", "Highway to Hell"), sql.Named("sqlair_1", 666)},
		"parameter issue: map type wrongMap must have key type string; found type int",
	},
	}
	for _, test := range testList {
		parser := expr.NewParser()
