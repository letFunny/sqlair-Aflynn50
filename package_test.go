...
func (s *PackageSuite) TestValidIterGet(c *C) {
	...
	err = db.Query(sqlair.MustPrepare(dropTables)).Run()
	if err != nil {
		c.Fatal(err)
	}
}
...
func (s *PackageSuite) TestGetErrors(c *C) {
	...
	for _, t := range tests {
		...
		err = db.Query(stmt, t.inputs...).Get(t.outputs...)
		c.Assert(err, ErrorMatches, t.err,
			Commentf("\ntest %q failed:\ninput: %s\noutputs: %s", t.summary, t.query, t.outputs))
	}
...
func (s *PackageSuite) TestGetAllErrors(c *C) {
	...
	for _, t := range tests {
		...
		err = db.Query(stmt, t.inputs...).GetAll(t.slices...)
		c.Assert(err, ErrorMatches, t.err,
...
func (s *PackageSuite) TestTransactions(c *C) {
	...
	err = tx.Query(insertStmt, &derek).Run()
	c.Assert(err, IsNil)
	err = tx.Rollback()
	c.Assert(err, IsNil)
	...
	err = tx.Query(selectStmt, &derek).Get(&derekCheck)
	if !errors.Is(err, sqlair.ErrNoRows) {
...
func (s *PackageSuite) TestIterMethodOrder(c *C) {
	...
	iter = db.Query(badTypesStmt).Iter()
	if !iter.Next() {
